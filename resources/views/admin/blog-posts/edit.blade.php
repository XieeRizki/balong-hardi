...
<div class="bg-white rounded-lg shadow p-8 max-w-4xl">
    <form action="{{ route('admin.blog-posts.update', $blogPost) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="category" value="{{ old('category', $blogPost->category) }}" class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-primary" required>
            @error('category') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="mb-6">
            <label class="block text-secondary font-bold mb-2">Konten Artikel</label>
            <textarea name="content" rows="10" class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-primary font-mono" required>{{ old('content', $blogPost->content) }}</textarea>
